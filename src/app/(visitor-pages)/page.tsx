...
const technologyIcons: Record<
  string,
  { icon: React.ComponentType<{ className?: string }>; category: string }
> = {
  // Languages
  ...
  // Design Tools
  "Adobe Photoshop": { icon: ImageLucide, category: "Design" },
  "Adobe Illustrator": { icon: PenTool, category: "Design" },
  "Adobe XD": { icon: LayoutGrid, category: "Design" },
  InDesign: { icon: BookOpen, category: "Design" },
  Sketch: { icon: SiSketch, category: "Design" },
  Canva: { icon: SiCanva, category: "Design" },
};

function TechnologyIconsGrid({ techNames }: { techNames: string[] }) {
  const technologies = Array.from(new Set(techNames));
  const categories = Array.from(
    new Set(
      technologies
        .map((tech) => technologyIcons[tech]?.category)
        .filter(Boolean),
    ),
  ).sort();
  ...
  return (
    ...
    >
      <Accordion
        ...
        className="w-full border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900 divide-y divide-zinc-300 dark:divide-zinc-700 min-w-full"
      >
        {categories.map((category) => {
          const categoryTechs = technologies.filter(
            (tech) => technologyIcons[tech]?.category === category,
          );
                    ...
                    return (
                      ...
                      >
                        <div className="relative flex flex-col items-center justify-center p-3 border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 hover:border-zinc-400 dark:hover:border-zinc-600 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all duration-300 cursor-default">
                          <IconComponent className="w-6 h-6 sm:w-7 sm:h-7 text-zinc-700 dark:text-zinc-300 group-hover:scale-110 transition-transform duration-300" />
...
export default async function HomePage() {
  const homeServices = await getHomeServices();
  const projectList = await getPublishedProjects();
  const testimonials = await getTestimonials();
  const socials = await getSocialLinks();
  const promos = await getPromoAds([PromoPlacement.HOME]);
  const homePromo = promos[0];

  const technologyNames = Array.from(
    new Set(
      homeServices.flatMap((s) =>
        Array.isArray(s.technologies) ? (s.technologies as string[]) : [],
      ),
    ),
  );
  const projectCards = projectList.map(projectToCard).slice(0, 4);
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center justify-start w-full scroll-smooth">
        <section
          id="home"
          className="flex flex-col mx-auto justify-center items-center w-full pt-48 pb-36 px-4 bg-zinc-900"
          style={jssStyles}
        >
          <BlurFade delay={0.25} className="mt-80" inView>
            <h1 className="text-center z-40">
              <span className="text-zinc-300 mb-10 text-2xl md:text-4xl block">
                Hello, I&apos;m
              </span>
              <span className="text-zinc-300 text-4xl md:text-6xl block">
                Jean Eric Hirwa
              </span>
            </h1>
          </BlurFade>
          <BlurFade delay={0.25 * 2} inView>
            <h2 className="text-center text-3xl md:text-5xl mt-10 text-zinc-300">
              A Full Stack Software Developer
            </h2>
          </BlurFade>
          <BlurFade delay={0.25 * 2} inView>
            <h3 className="text-center text-xl text-wrap md:text-2xl mt-4 text-zinc-300">
        ...
        <section
          id="skills"
          className="max-w-screen-xl flex flex-col mx-auto justify-between items-start w-full px-4 pt-24 bg-zinc-900"
        >
          <div className="flex flex-col gap-8 w-full">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
              {homeServices.map((service, index) => (
                <div
                  key={service.id}
                  className="border border-zinc-800 flex flex-col justify-between dark:border-zinc-600 p-8 md:p-12 bg-zinc-800"
                >
                  <div>
                    <span className="text-zinc-300">0{index + 1}</span>
                    <h4 className="text-2xl uppercase text-zinc-300">
                      {service.name}
                    </h4>
                    ...
                    <ul className="flex flex-wrap gap-2">
                      {(Array.isArray(service.technologies)
                        ...
                        <li
                          className="text-sm list-inside px-2 py-0 text-zinc-200 hover:bg-zinc-200 hover:dark:bg-zinc-600 bg-zinc-600 hover:text-zinc-900 transition-all duration-300"
                          key={technology}
                        >
                          {technology}
                        </li>
                  ...
                  <Link
                    ...
                    className="flex items-center justify-start gap-4 mt-12 group"
                  >
                    <span className="border border-zinc-500 p-2 rounded-full bg-zinc-800 hover:bg-zinc-700 transition-all duration-300">
                      <ArrowUpIcon className="rotate-45 group-hover:rotate-90 transition-transform duration-300 text-zinc-300 group-hover:text-zinc-900" />
                    </span>
                    <span className="text-sm text-zinc-300 group-hover:text-zinc-900 transition-all duration-300">
        ...
        <section
          id="works"
          className="max-w-screen-xl flex flex-col mx-auto justify-start items-start w-full pt-32 pb-12 lg:pb-32 px-4 bg-zinc-900"
        >
          <div className="flex justify-between items-center w-full ">
            <h3 className="text-3xl md:text-4xl text-zinc-300">
              Selected Work
            </h3>
            <Link
              href="/work"
            ...
            >
              ...
              <span className="text-center text-base md:text-xl text-zinc-300 group-hover:text-zinc-900 transition-all duration-300">
                SEE ALL
              </span>
            </Link>
          </div>
        ...
        {/* <section id="process" className="max-w-screen-xl flex flex-col mx-auto justify-center items-center w-full px-4 bg-zinc-900">
                    <h3 className="text-lg font-semibold text-zinc-300">THE PROCESS</h3>
                    <h4 className="text-6xl md:text-8xl flex flex-col font-extralight items-center text-zinc-300">
                        <span className="text-center">Your next project</span>
                        <em className="pt-serif-regular-italic"> in 5 steps</em>
                    </h4>
                    <div className="flex flex-col justify-between items-center gap-8">
                        <span className="border border-zinc-500 mt-8 p-4 rounded-full bg-zinc-800 hover:bg-zinc-700 transition-all duration-300">
                            <ArrowDownIcon className="text-zinc-300" />
                        </span>
                        <WorkProcess />
                    </div>
                    <BlurFade delay={0.1} inView className="w-full">
                        <div className="flex flex-col gap-12 w-full mt-12">
                            <div className="flex justify-between items-center w-full flex-wrap gap-4">
                                <h3 className="text-3xl md:text-4xl text-zinc-300">Technologies I work With</h3>
                            ...
                            <TechnologyIconsGrid techNames={technologyNames} />
                        </div>
                    </BlurFade>
                </section> */}
        <section className="max-w-screen-xl flex flex-col mx-auto py-12 lg:py-36 justify-center items-center w-full px-4 bg-zinc-900">
          <div className="gap-4 flex flex-col justify-between items-center">
            <h3 className="text-6xl md:text-8xl flex flex-col font-extralight items-center text-zinc-300">
        ...
        <section className="flex flex-col mx-auto justify-center items-center w-full gap-32 pb-32 max-w-screen-xl px-4 bg-zinc-900">
          {homePromo ? (
            <div className="w-full mb-16">
              <PromoBlock
                kicker={homePromo.kicker}
                headlineLead={homePromo.headlineLead}
                headlineEmphasis={homePromo.headlineEmphasis}
                body={homePromo.body}
                ctaLabel={homePromo.ctaLabel}
                ctaUrl={homePromo.ctaUrl}
              />
            </div>
          ) : null}
          <div
          ...
          >
            <p className="text-8xl w-full md:w-1/2 text-center md:text-start md:text-9xl mb-0 pt-serif-regular-italic font-light text-zinc-300">
              Jean Eric
            </p>
            <p className="text-8xl w-full md:w-1/2 text-center md:text-end md:text-9xl mt-0 font-light text-zinc-300">
              Hirwa
            </p>
          </div>
          <div className="max-w-screen-xl px-4 flex w-full justify-between items-start flex-wrap md:flex-nowrap gap-8">
            <h3 className="w-full md:w-[48%] text-4xl md:text-5xl flex flex-col font-extralight text-zinc-300">
            ...
            <div className="w-full md:w-[48%] flex flex-col justify-start items-start gap-8">
              <p className="text-lg text-zinc-300">
                ...
                enjoy combining my tech skills with my design background to
                create innovative solutions. Currently, I&apos;m working on
                empowering women through tech at IRO. Outside of work, I love
                exploring new ideas, dancing, and diving into a good book or
                movie.
              </p>
              <div className="flex justify-start items-start gap-3">
                {socials.map((account, index) => (
                  ...
                  >
                    <span className="border border-zinc-500 p-3 rounded-full bg-zinc-800 hover:bg-zinc-700 transition-all duration-300">
                      {account.name === "Github" && (
                        <GitHubLogoIcon className="w-6 h-6 text-zinc-300" />
                      )}
                      {account.name === "LinkedIn" && (
                        <LinkedInLogoIcon className="w-6 h-6 text-zinc-300" />
                      )}
                      {account.name === "Instagram" && (
                        <InstagramLogoIcon className="w-6 h-6 text-zinc-300" />
                      )}
                      {account.name === "Medium" && (
                        <BsMedium className="w-6 h-6 text-zinc-300" />
                      )}
                      {account.name === "YouTube" && (
                        <FaYoutube className="w-6 h-6 text-zinc-300" />
                      )}
                    </span>
                  </Link>
                ))}
