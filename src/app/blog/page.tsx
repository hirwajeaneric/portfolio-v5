  ...
  return (
    <div className="flex flex-col items-center justify-start w-full scroll-smooth">
      <section id="process" className="max-w-screen-xl flex flex-col mx-auto justify-center items-center pt-40 md:pt-48 pb-16 md:pb-32 w-full px-4">
        ...
        <p className="text-center mt-4 md:mt-8 mb-12 md:mb-18 text-lg w-5/6 md:w-1/2">Get the latest insights on some topics I find attractive.</p>
        <div className="grid grid-cols-1 smd:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-10 w-full">
          {Articles.map((article, index) => (
            <Link href={`/blog/${article.slug}`} key={index} className="cursor-pointer flex flex-col p-10 justify-between gap-8 w-full bg-zinc-800 border border-zinc-500 group">
