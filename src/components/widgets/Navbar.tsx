...
export default function Navbar() {
    ...
    return (
        <header className="flex flex-col z-40 fixed mx-auto w-[92%] justify-between items-center top-6 gap-2">
            <div className="flex cursor-pointer items-center justify-between space-x-8 border w-full md:w-fit px-4 py-2 bg-zinc-800 border-zinc-600">
                <Link href={'/'} onClick={() => setOpen(false)} className="font-bold text-xl leading-tight text-nowrap text-zinc-300 hover:text-zinc-100">Jean Eric Hirwa</Link>
                <div className="flex justify-between items-center gap-6">
                    <nav className="justify-between hidden md:flex gap-6 text-sm">
                        <Link href={'/services'} className={cn(pathname === "/services" ? "border-b border-zinc-600 dark:border-zinc-300" : "", "hover:border-b border-zinc-800 dark:border-zinc-300 text-zinc-300 hover:text-zinc-100")}>SERVICES</Link>
                        <Link href={'/work'} className={cn(pathname === "/work" ? "border-b border-zinc-600 dark:border-zinc-300" : "", "hover:border-b border-zinc-800 dark:border-zinc-300 text-zinc-300 hover:text-zinc-100")}>WORK</Link>
                        <Link href={'/about'} className={cn(pathname === "/about" ? "border-b border-zinc-600 dark:border-zinc-300" : "", "hover:border-b border-zinc-800 dark:border-zinc-300 text-zinc-300 hover:text-zinc-100")}>ABOUT</Link>
                        <Link href={'/blog'} className={cn(pathname === "/blog" ? "border-b border-zinc-600 dark:border-zinc-300" : "", "hover:border-b border-zinc-800 dark:border-zinc-300 text-zinc-300 hover:text-zinc-100")}>BLOG</Link>
                    </nav>
                    <Link href={'/contact'} className="px-4 py-3 bg-zinc-300 text-zinc-900 hover:bg-zinc-700 hover:text-zinc-300 text-nowrap text-sm">LET&apos;S TALK</Link>
